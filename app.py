...
with all_tabs[2]:
    ...
    st.caption("※本規約はクローズドテストおよび将来のFlutter移植・商用リリースを見据えた法務防衛線です。")
    
    st.info(
        "⚖️ **【免責事項・利用上の注意】**\n"
        "1. **AI出力の性質：** 当アプリのコンシェルジュ（ハヤト等）が紡ぐ言葉は、大規模言語モデルによって自律的に生成された回答であり、その正確性、完全性、医療的・技術的妥当性を100%保証するものではありません。専門的な判断を要する情報については、必ず専門家にご確認ください。\n"
        "2. **データの取扱（長期記憶カルテ）：** ユーザーとコンシェルジュとの会話履歴は、高度な長期記憶システム（ハイブリッド検索および自動要約機能）のために暗号化された上でデータベースへ保存されます。プライバシーは厳重に保護されますが、テスター期間中の品質向上のため、管理者が監査ログ（エラー解析等）を確認する場合があります。\n"
