import streamlit as st
from google import genai
import os
import re

import os
from supabase import create_client, Client

# Supabaseの接続情報（※ご自身のURLとPublishable keyに書き換えてください）
SUPABASE_URL = "https://mulkgkhozkvjmjdlfzwu.supabase.co"
SUPABASE_KEY = "sb_publishable_HOdI0Fp8SMVLBDG_IJ-qiw_5rBl3Ces"

# クライアント（通信用ロボット）の初期化
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)



# ==========================================
# 【重要】あなたのAPIキーをここに貼り付けてください
API_KEY = "AQ.Ab8RN6JHvTGeeRAdCrrAWzh8SsmEUM6Iv0UGrFzgwr2YewsWxQ"
# ==========================================

# 保存するファイルのパス（デスクトップの chat_log.txt）
LOG_FILE = os.path.join(os.path.expanduser("~"), "Desktop", "chat_log.txt")

st.title("個別相談ＡＩ")
st.caption("※過去の会話をすべて読み込んで続きから再開できます")

# AIクライアントの初期化
client = genai.Client(api_key=API_KEY)

# 【新機能】起動時に、テキストファイルから過去の会話を自動で読み込んで復活させる
if "messages" not in st.session_state:
    st.session_state.messages = []
    
    if os.path.exists(LOG_FILE):
        try:
            with open(LOG_FILE, "r", encoding="utf-8") as f:
                content = f.read()
            
            # テキストファイルから【あなた】と【AI】の会話を切り出す
            blocks = content.split("--- 会話の記録 ---")
            for block in blocks:
                user_match = re.search(r"【あなた】:\s*(.*)", block)
                ai_match = re.search(r"【AI】:\s*(.*)", block)
                
                if user_match and ai_match:
                    st.session_state.messages.append({"role": "user", "content": user_match.group(1).strip()})
                    st.session_state.messages.append({"role": "assistant", "content": ai_match.group(1).strip()})
        except Exception as e:
            st.error(f"過去ログの読み込みエラー: {e}")

# 過去の会話を青とロボットのアイコンで画面に表示
for message in st.session_state.messages:
    avatar = "🔵" if message["role"] == "user" else "🤖"
    with st.chat_message(message["role"], avatar=avatar):
        st.write(message["content"])

# ユーザーからの入力
if user_input := st.chat_input("AIに相談したいことを入力してください..."):
    # ユーザーの入力を表示
    with st.chat_message("user", avatar="🔵"):
        st.write(user_input)
    st.session_state.messages.append({"role": "user", "content": user_input})

    # AIにこれまでの全履歴をセットにして送る
    chat_contents = []
    for msg in st.session_state.messages:
        role_name = "user" if msg["role"] == "user" else "model"
        chat_contents.append({"role": role_name, "parts": [{"text": msg["content"]}]})

    # AIの返答を取得
    try:
        response = client.models.generate_content(
            model='gemini-3.5-flash-lite',
            contents=chat_contents,
        )
        ai_response = response.text
    except Exception as e:
        ai_response = f"エラーが発生しました: {e}"

    # AIの返答を表示
    with st.chat_message("assistant", avatar="🤖"):
        st.write(ai_response)
    st.session_state.messages.append({"role": "assistant", "content": ai_response})

    # 会話をテキストファイルに保存（追記）
    try:
        with open(LOG_FILE, "a", encoding="utf-8") as f:
            f.write(f"--- 会話の記録 ---\n")
            f.write(f"【あなた】: {user_input}\n")
            f.write(f"【AI】: {ai_response}\n\n")
    except Exception as e:
        st.error(f"ファイル保存エラー: {e}")

# === Supabase（クラウド）へ保存するコード ===
    try:
        # ユーザーの発言を保存
        supabase.table("chat_history").insert({"role": "user", "content": user_input}).execute()
        # AIの返答を保存
        supabase.table("chat_history").insert({"role": "assistant", "content": ai_response}).execute()
    except Exception as e:
        st.error(f"クラウド保存エラー: {e}")


st.markdown(
    """
    <style>
    /* チャット内の見出し（###など）の大きさを指定 */
    .stChatMessage h1, .stChatMessage h2, .stChatMessage h3, .stChatMessage h4 {
        font-size: 18.5px !important;
        font-weight: bold !important;
        margin-top: 14px !important;
        margin-bottom: 6px !important;
    }
    /* 本文の中の太字（**の部分）を正しく太字として表示させる記述を追加 */
    .stChatMessage p strong {
        font-weight: bold !important;
        font-size: inherit !important;
    }
    </style>
    """,
    unsafe_allow_html=True
)
