...
MODE_PROMPTS = {
    ...
    【重要】
    ・通常は結論と要点のみ回答してください。
    ・詳細な前提条件、計算式、内訳、比較表は、ユーザーが明示的に求めた場合のみ提示してください。
    ・ユーザーが条件変更のみを行った場合は、変更後の結論と前回との差分を優先して回答してください。
    ・過去の会話で既に説明済みの前提条件は、変更がない限り繰り返し説明しないでください。

    ・条件変更、金額変更など、既存の試算条件の一部だけが変更された場合は、変更後の結論と前回との差分を優先して説明してください。
