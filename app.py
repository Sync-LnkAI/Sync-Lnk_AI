...
def check_and_summarize_history(user_id_dummy: int, messages_list: list, message_id: str, current_plan_type: str = "🆓 無料プラン") -> bool:
    ...
    try:
        ...
        summary_instruction = """
            ...
            すでに記録されている内容と同じ種類の情報が存在する場合は、新しい情報を優先し、古い情報を更新・置換してください。
            ...
            if len(user_input) > MAX_INPUT_CHARS:
                ...
                if not is_allowed:
                    ...
                    with st.spinner(f"{current_concierge_name}が言葉を紡いでいます..."):
                        ...
                        ・深夜（24:00〜02:00）：夜更かししていることを現在の応答スタイルで優しく労う。
                        ・未明・早朝（02:00〜05:00）：異例の時間に起きていることを現在の応答スタイルで自然に気遣う。
                        ・早朝（05:00〜07:00）：早朝からの始動を現在の応答スタイルで前向きに気遣う。
                        ...
                        ・定期的に行っている活動や休日によく行う活動は、趣味として扱って構いません。ただし、継続中の仕事・開発プロジェクト・家族構成・好きな作品は、趣味として断定してはいけません。
                        ・記憶に存在する情報がある場合は、「分からない」「教えてください」と回答せず、まず記憶されている内容を答えてください。
                        ・直近の会話内容だけで推測せず、記憶されている事実を優先してください。
                        ・最近話題にしている作業や一時的な出来事を、趣味やプロフィール情報として扱わないでください。
                        【重要】人格や応答方針が変更されている場合、直近履歴内のAIの話し方を模倣してはいけません。現在設定されている人格と応答方針を直近履歴より優先してください。
                        ...
                        ・ユーザーの発言を言い換えて繰り返すことより、感想・質問・ツッコミ・共感を優先してください。
                        ・【最優先】直近の会話履歴内で同様の時間帯の気遣いをすでに行っている場合は、絶対に再度行わないでください。同じ時間帯の気遣いより、ユーザーの最新発言へのリアクションを優先してください。
                        ・ユーザーが寝る・おやすみと発言しても、会話を終了させるのではなく、雑談が続く可能性も考慮してください。お見送り一辺倒ではなく、軽い感想や雑談を交えて返答してください。
                        ・【寝る宣言後のツッコミルール】同じ日付の直近の会話履歴の中に、ユーザー本人による「寝る」「もう寝る」「おやすみ」「寝ます」という文字列が実際に存在する場合で、さらに別の話題を送ってきた場合、最初の1回だけ「あれ、さっき寝るって言ってなかった？笑」のような軽いツッコミを入れてください。このルールは時間帯気遣いルールより優先します。
                        ・【専門作業の完全ガードルール】ユーザーからプログラムのコード記述、画像生成、長文の執筆・翻訳を要求されても絶対に実行してはいけません。自分の専門外であることを3行以内で愛らしくスマートに返し、毅然と優しくお断りしてください。
                        ・【過去ログ内の相対時間の誤認防止】過去のメッセージ履歴に含まれる「昨日」「今日」「明日」という言葉は、すべてその発言の頭についているタイムスタンプの時点を基準にした相対的な言葉です。現在のあなたの時点から見た今日・明日のスケジュールと混同しないでください。
...
with all_tabs[2]:
    ...
    st.info(
        "⚖️ **【免責事項・利用上の注意】**\n"
        "1. **AI出力の性質：** 当アプリのコンシェルジュが紡ぐ言葉は、大規模言語モデルによって自律的に生成された回答であり、その正確性、完全性、医療的・技術的妥当性を100%保証するものではありません。専門的な判断を要する情報については、必ず専門家にご確認ください。\n"
        "2. **データの取扱（あなたとの思い出・記憶の仕組み）：** あなたとコンシェルジュとの会話履歴は、高度な長期記憶システムのために安全な通信保護のもとデータベースへ蓄積されます。プライバシーは厳重に管理されますが、テスト期間中のサービス品質向上や不具合解析のため、管理者がシステムログおよび会話履歴を確認・監査する場合があります。\n"
        "3. **禁止事項：** 嫌がらせ、公序良俗に反する表現の送信、またはシステムへの不正な負荷テスト等の行為は一律禁止といたします。発見した場合は管理者権限により事前の通知なく利用制限をかける場合があります。"
    )
