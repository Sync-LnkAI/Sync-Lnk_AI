...
# 起動時にSupabaseからデータを確実に全件読み込む処理
if current_topic_key not in st.session_state:
    st.session_state[current_topic_key] = []
    try:
        # Supabaseから全データを取得
        response = supabase.table("chat_history").select("role, content, topic").order("created_at", desc=False).execute()
        data = response.data

        if data:
            for row in data:
                db_topic = row.get("topic")
                # 「過去の会話ログ（全般）」が選ばれている時は、旧データ(None/空)または一致するデータをすべて追加
                if selected_topic == "📜 過去の会話ログ（全般）":
                    if db_topic == selected_topic or db_topic is None or db_topic == "":
                        st.session_state[current_topic_key].append({"role": row["role"], "content": row["content"]})
                else:
                    # それ以外のトピックは完全一致のみ追加
                    if db_topic == selected_topic:
                        st.session_state[current_topic_key].append({"role": row["role"], "content": row["content"]})
